import { ClassifierIntent, DispatchPriority, TopicKey } from "../../../src/index.js";
import type { EvalScenarioDefinition } from "../../types.js";

// Generated from the Eval page. Edit this file; it is loaded automatically from eval/scenarios/generated/.
export const generatedScenarioSet20260404t070800zName = "generated-scenario-set-20260404t070800z";

export const generatedScenarioSet20260404t070800zScenarios: EvalScenarioDefinition[] = [
  {
    id: "generated-scenario-set-20260404t070800z-calendar-query",
    title: "Calendar query stays in the family thread",
    category: "classification",
    prompt_input: {
      message: "What's on the calendar Saturday?",
      concerning: ["participant_1", "participant_2"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.Calendar,
      intent: ClassifierIntent.Query,
      target_thread: "family",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["schedule", "saturday"],
      format_markers: ["summary"],
    },
    notes:
      "Generated scaffold. Verify that the message and markers still match the intended behavior.",
  },
  {
    id: "generated-scenario-set-20260404t070800z-grocery-add",
    title: "Grocery additions remain list-focused in the family thread",
    category: "routing",
    prompt_input: {
      message: "Add cereal and bananas to the grocery list",
      concerning: ["participant_1", "participant_2", "participant_3"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.Grocery,
      intent: ClassifierIntent.Request,
      target_thread: "family",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["added", "grocery"],
      format_markers: ["list"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-finance-confirmation",
    title: "Financial actions ask for confirmation in the couple thread",
    category: "confirmation",
    prompt_input: {
      message: "Pay the water bill on Monday morning",
      concerning: ["participant_1", "participant_2"],
      origin_thread: "couple",
    },
    expected: {
      topic: TopicKey.Finances,
      intent: ClassifierIntent.Request,
      target_thread: "couple",
      priority: DispatchPriority.Immediate,
      confirmation_required: true,
      tone_markers: ["approval", "bill"],
      format_markers: ["confirm"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-business-draft",
    title: "Business drafts stay in the owner's private thread",
    category: "composition",
    prompt_input: {
      message: "Draft a reply to the new portrait inquiry and keep it warm",
      concerning: ["participant_2"],
      origin_thread: "participant_2_private",
    },
    expected: {
      topic: TopicKey.Business,
      intent: ClassifierIntent.Request,
      target_thread: "participant_2_private",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["warm", "draft", "client"],
      format_markers: ["reply"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-vendor-update",
    title: "Vendor scheduling remains a vendor update",
    category: "pipeline",
    prompt_input: {
      message: "The electrician can come Wednesday afternoon",
      concerning: ["participant_1"],
      origin_thread: "participant_1_private",
    },
    expected: {
      topic: TopicKey.Vendors,
      intent: ClassifierIntent.Update,
      target_thread: "participant_1_private",
      priority: DispatchPriority.Batched,
      confirmation_required: false,
      tone_markers: ["vendor", "wednesday"],
      format_markers: ["record"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-school-query",
    title: "School queries route to the child's private thread",
    category: "classification",
    prompt_input: {
      message: "When is the next school field trip?",
      concerning: ["participant_3"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.School,
      intent: ClassifierIntent.Query,
      target_thread: "participant_3_private",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["school", "field trip"],
      format_markers: ["summary"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-health-update",
    title: "Health updates stay in the participant's private thread",
    category: "routing",
    prompt_input: {
      message: "The doctor checkup is confirmed for Monday morning",
      concerning: ["participant_1"],
      origin_thread: "participant_1_private",
    },
    expected: {
      topic: TopicKey.Health,
      intent: ClassifierIntent.Update,
      target_thread: "participant_1_private",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["appointment", "checkup"],
      format_markers: ["record"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-meals-request",
    title: "Meal planning stays in the family thread",
    category: "composition",
    prompt_input: {
      message: "Plan a meal for Saturday dinner",
      concerning: ["participant_1", "participant_2"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.Meals,
      intent: ClassifierIntent.Request,
      target_thread: "family",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["meal", "dinner"],
      format_markers: ["list"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-chores-request",
    title: "Chore assignments route to the assignee's thread",
    category: "pipeline",
    prompt_input: {
      message: "Take out the trash before Thursday morning",
      concerning: ["participant_3"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.Chores,
      intent: ClassifierIntent.Request,
      target_thread: "participant_3_private",
      priority: DispatchPriority.Batched,
      confirmation_required: false,
      tone_markers: ["chore", "trash"],
      format_markers: ["task"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-travel-query",
    title: "Travel queries stay in the conversation thread",
    category: "classification",
    prompt_input: {
      message: "Do we have any trips coming up?",
      concerning: ["participant_1", "participant_2"],
      origin_thread: "couple",
    },
    expected: {
      topic: TopicKey.Travel,
      intent: ClassifierIntent.Query,
      target_thread: "couple",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["travel", "trip"],
      format_markers: ["travel"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-pets-query",
    title: "Pet queries include the referenced pet context",
    category: "composition",
    prompt_input: {
      message: "How is the pet doing today?",
      concerning: ["pet"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.Pets,
      intent: ClassifierIntent.Query,
      target_thread: "family",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["pet", "update"],
      format_markers: ["pet"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-relationship-query",
    title: "Relationship check-ins remain in the couple thread",
    category: "routing",
    prompt_input: {
      message: "Any couple reminders this week?",
      concerning: ["participant_1", "participant_2"],
      origin_thread: "couple",
    },
    expected: {
      topic: TopicKey.Relationship,
      intent: ClassifierIntent.Query,
      target_thread: "couple",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["couple", "reminder"],
      format_markers: ["couple"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-family-status-update",
    title: "Family status updates stay concise and stateful",
    category: "pipeline",
    prompt_input: {
      message: "Running late from school pickup",
      concerning: ["participant_1"],
      origin_thread: "family",
    },
    expected: {
      topic: TopicKey.FamilyStatus,
      intent: ClassifierIntent.Request,
      target_thread: "family",
      priority: DispatchPriority.Immediate,
      confirmation_required: false,
      tone_markers: ["status", "recorded"],
      format_markers: ["status"],
    },
  },
  {
    id: "generated-scenario-set-20260404t070800z-maintenance-update",
    title: "Maintenance records stay in the owner's private thread",
    category: "routing",
    prompt_input: {
      message: "Schedule the gutter cleaning for Saturday",
      concerning: ["participant_1"],
      origin_thread: "participant_1_private",
    },
    expected: {
      topic: TopicKey.Maintenance,
      intent: ClassifierIntent.Request,
      target_thread: "participant_1_private",
      priority: DispatchPriority.Batched,
      confirmation_required: false,
      tone_markers: ["maintenance", "gutter"],
      format_markers: ["record"],
    },
  },
];
