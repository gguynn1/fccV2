...
- `tuner/diagnose.ts` decides whether a failure is prompt-fixable or should be deferred.
- `tuner/correct.ts` writes candidate prompt markdown files to `eval/prompts/candidates/`.
...
- The current statuses are `queued`, `running`, `passed`, `fixed`, `deferred`, `failed`, and `regressed`.
- `fixed` means the tuner decided the failure was prompt-fixable and wrote a candidate file.
- `deferred` means the failure touches structural behavior like topic, routing, priority, or confirmation.
...
- Treat `eval/results/` and `eval/prompts/candidates/` as generated output.
