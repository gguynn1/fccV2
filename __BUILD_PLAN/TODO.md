...
- [x] crash restart behavior is confirmed
- [ ] reboot auto-start behavior is confirmed
- [ ] power and login settings required by the hosting model are confirmed
- [x] SQLite backup job is scheduled and Redis AOF files are included in backup scope
...
- [x] CalDAV `PROPFIND`, `REPORT`, and `GET` smoke tests pass
- [ ] local-network calendar subscription renders expected events

### Admin UI Verification

- [x] `/admin` is served correctly after UI build
- [x] forwarded-header requests to `/admin` and `/api/admin/*` are rejected
- [x] all admin pages work against the running backend
- [x] top navigation active state is correct across routes
